...
/////////////////////////////////////
chrome.storage.sync.set({'stylesheet-version': 0}); // FLAG FOR REVIEW - remove
...
function startDeleteElement() {
    ...
    exitFunc = function() {
        ...
        jrContentContainer.removeEventListener('mouseover', mouseFunc);
        jrContentContainer.removeEventListener('click', clickFunc);
        jrContentContainer.removeEventListener('keydown', escFunc);

        [...iframes].forEach(elem => elem.style.pointerEvents = "auto");

        if(jrContentContainer.querySelector(".jr-hovered") != null)
            jrContentContainer.querySelector(".jr-hovered").classList.remove("jr-hovered");

        jrContentContainer.classList.remove("simple-deleting");
    ...
    var anchors = jrContentContainer.querySelectorAll("a");
    anchors.forEach(function(a) {
        a.addEventListener("click", anchorFunc);
    });

    jrContentContainer.classList.add("simple-deleting");

    jrContentContainer.addEventListener('mouseover', mouseFunc);
    jrContentContainer.addEventListener('click', clickFunc);
    jrContentContainer.addEventListener('keydown', escFunc);

    var iframes = jrContentContainer.querySelectorAll("iframe");
    [...iframes].forEach(elem => elem.style.pointerEvents = "none");

    var sd = jrContentContainer.querySelector(".simple-delete");
...
function updateSavedVersion() {
    if(chromeStorage["backup"]) {
        chrome.runtime.sendMessage({ savedVersion: jrContentContainer.querySelector('.content-container').innerHTML });
    }
}
...
function closeOverlay() {
    ...
    // Fade out
    jrHost.classList.add("simple-fade-up");

    // Reset our variables
    jrContentContainer = undefined;
...
function getContainer() {
    ...
    if(ps.length === 0) {
        ps = document.querySelectorAll("div");
    }
...
function linkListener(e) {
    if(!jrContentContainer.classList.contains("simple-deleting")) {
        ...
        || (hrefArr[0].replace(/\/$/, "") != top.window.location.origin + top.window.location.pathname.replace(/\/$/, "") // Anchored to an ID on another page
            ...
            && hrefArr[0] != "_blank")
        || (jrContentContainer.getElementById(hrefArr[1]) == null // The element is not in the article section
            && jrContentContainer.querySelector("a[name='" + hrefArr[1] + "']") == null)
        ) {
            top.window.location.href = this.href; // Regular link
        } else { // Anchored to an element in the article
            top.window.location.hash = hrefArr[1]; // FLAG FOR REVIEW
        }
    }
}
...
function getStyles() {
    ...
    chrome.storage.sync.get(null, function (result) {
        ...
        if(typeof versionResult['stylesheet-version'] === "undefined"
...
function addStylesheet(elem, link, classN) {
...
var StyleEditor = function() {
    ...
    this.fontSize = prevStyles.fontSize = getStylesheetValue(s, ".jr-body", "font-size");
    this.textColor = prevStyles.textColor = getStylesheetValue(s, ".jr-body", "color");
    this.backgroundColor = prevStyles.backgroundColor = getStylesheetValue(s, ".jr-body", "background-color");
...
function updateEditorStyles(editor) {
    editor.fontSize = getStylesheetValue(s, ".jr-body", "font-size");
    editor.textColor = getStylesheetValue(s, ".jr-body", "color");
    editor.backgroundColor = getStylesheetValue(s, ".jr-body", "background-color");
...
function updatePrevStyles(theme) {
    ...
    prevStyles.fontSize = getStylesheetValue(s, ".jr-body", "font-size");
    prevStyles.textColor = getStylesheetValue(s, ".jr-body", "color");
    prevStyles.backgroundColor = getStylesheetValue(s, ".jr-body", "background-color");
...
function closeStyleEditor() {
    if(!saved) {
        changeStylesheetRule(s, ".jr-body", "font-size", prevStyles.fontSize);
        changeStylesheetRule(s, ".simple-container", "max-width", prevStyles.maxWidth);
        changeStylesheetRule(s, ".jr-body", "color", prevStyles.textColor);
        changeStylesheetRule(s, ".jr-body", "background-color", prevStyles.backgroundColor);
...
function openStyleEditor() {
    s = jrHost.shadowRoot.styleSheets[2];
    ...
    if(datGUI) {
        ...
        fontSize.onChange((value) => {
            saved = false;
            changeStylesheetRule(s, ".jr-body", "font-size", value);
        });
        ...
        textColor.onChange((value) => {
            saved = false;
            changeStylesheetRule(s, ".jr-body", "color", value);
        });
        let backgroundColor = datGUI.addColor(editor, 'backgroundColor');
        backgroundColor.onChange((value) => {
            saved = false;
            changeStylesheetRule(s, ".jr-body", "background-color", value);
        });
...
function createSimplifiedOverlay() {
    ...
    jrHost.className = "simple-fade-up no-trans"; // Add fade
    // Add our shadow container to the page
    document.body.appendChild(jrHost);

    var shadow = jrHost.attachShadow({mode: 'open'});

    // Create a "body" to use for the shadow content
    let jrBody = document.createElement("div");
    jrBody.className = "jr-body";
    shadow.appendChild(jrBody);

    jrContentContainer = document.createElement("div");
    ...
    if(direction === "rtl" || isRTL(contentContainer.firstChild.innerText)) {
        jrContentContainer.classList.add("rtl");
    }

    jrContentContainer.appendChild(contentContainer);

    // Remove the elements we flagged earlier
    var deleteObjs = jrContentContainer.querySelectorAll("[data-simple-delete]");
    ...
    jrContentContainer.appendChild(addExtInfo());
    ...
    if(chromeStorage['addTimeEstimate']) {
        let wordCount = jrHost.shadowRoot.querySelector(".content-container").innerHTML.split(/\s+/).length;
        jrHost.shadowRoot.querySelector(".simple-time-estimate").innerText = Math.floor(wordCount / 200) + ' minute read';
    }
    ...
    jrContentContainer.appendChild(uiContainer);

    // Add the notification of premium if necessary
    if((jrCount === 5
    || jrCount % 15 === 0)
    && jrCount < 151) {
        jrContentContainer.appendChild(addPremiumNofifier());
    }
    ...
    if(mj) {
        ...
        for(var i = 0; i < scripts.length; i++) {
            if(scripts[i].innerText.indexOf("MathJax.Hub.Config") >= 0) {
                ...
                jrContentContainer.appendChild(clone);
            }
        }
    }
    ...
    jrHost.shadowRoot.querySelector(".simple-close").addEventListener('click', closeOverlay);

    // The print button
    jrHost.shadowRoot.querySelector(".simple-print").addEventListener('click', function() {
        window.print();
    });

    // The deletion mode button
    var sd = jrHost.shadowRoot.querySelector(".simple-delete");
    if(sd) {
        sd.onclick = function() {
            startDeleteElement(document);
        };
    }
    ...
    document.addEventListener("keydown", function(e) {
        // Listen for the "Esc" key and exit if so
        if(e.keyCode === 27 && !document.body.classList.contains("simple-deleting") && document.hasFocus())
            closeOverlay();
    ...
    var youtubeFrames = jrHost.shadowRoot.querySelectorAll("iframe[src *= 'youtube.com/embed/']");
...
function finishLoading() {
    ...
    if(!jrHost.shadowRoot.querySelector(".required-styles"))
        addStylesheet(jrContentContainer, "required-styles.css", "required-styles");

    // Add the segments hider if needed
    if((chromeStorage['hideSegments']
        && !jrHost.shadowRoot.querySelector(".hide-segments"))
    || typeof chromeStorage['hideSegments'] === "undefined") {
        addStylesheet(jrContentContainer, "hide-segments.css", "hide-segments");
    }

    // Change the top most page when regular links are clicked
    var links = jrHost.shadowRoot.querySelectorAll("a");
    var linkNum = links.length;
    for(var i = 0; i < linkNum; i++)
        links[i].onclick = linkListener;

    // Navigate to the element specified by the URL # if it exists
    if(top.window.location.hash != null)
        setTimeout(function () { window.location.hash = top.window.location.hash; }, 10); // FLAG FOR REVIEW

    // Append our theme styles to the overlay
    jrContentContainer.appendChild(styleElem);
...
function fadeIn() {
    if(jrContentContainer.querySelector(".required-styles")) {
        jrHost.classList.remove("no-trans");
        jrHost.classList.remove("simple-fade-up");
    } else {
...
var isPaused = false,
    stylesheetObj = {},
    stylesheetVersion = 2.0; // THIS NUMBER MUST BE CHANGED FOR THE STYLESHEETS TO KNOW TO UPDATE
