...
function updateEditorStyles(editor) {
  ...
  editor.linkHoverColor = getStylesheetValue(
    JR.themeStylesheet,
    "a[href]:hover",
    "color",
  );
...
function persistCurrentTheme(theme) {
  JR.theme = theme;
  JR.settings.currentTheme = theme;
  chrome.storage.sync.set({ currentTheme: theme });
}

function syncThemeSelectionFromEditor() {
  const selectedTheme = JR.styleEditor?.theme;
  if (selectedTheme && selectedTheme !== themeAtOpen) {
    persistCurrentTheme(selectedTheme);
  }
}

function updatePrevStyles(newTheme) {
  prevStyles.theme = newTheme;
  prevStyles.fontSize = getStylesheetValue(
    JR.themeStylesheet,
    bodySelector,
    "font-size",
  );
  prevStyles.textColor = getStylesheetValue(
    JR.themeStylesheet,
    bodySelector,
    "color",
  );
  ...
  prevStyles.linkColor = getStylesheetValue(
    JR.themeStylesheet,
    "a[href]",
    "color",
  );
  prevStyles.linkHoverColor = getStylesheetValue(
    JR.themeStylesheet,
    "a[href]:hover",
    "color",
  );
...
function saveStyles() {
  ...
  if (JR.theme === "default-styles.css" || JR.theme === "dark-styles.css") {
    ...
    persistCurrentTheme(JR.theme);
    newTheme = true;
  }

  let CSSString = "";
  Array.from(JR.themeStylesheet.cssRules).forEach(
    (rule) => (CSSString += rule.cssText + "\n"),
  );
...
function closeStyleEditor() {
  syncThemeSelectionFromEditor();

  if (!saved && stylesModified) {
...
function openStyleEditor() {
  ...
  themeAtOpen = JR.theme;

  if (JR.datGUI) {
    JR.datGUI.domElement.style.display = "block";
    JR.datGUI.closed = false;
    JR.themeStylesheet = getThemeStylesheet();
    if (JR.styleEditor && JR.themeStylesheet) {
      JR.styleEditor.theme = JR.theme;
      updateEditorStyles(JR.styleEditor);
      updatePrevStyles(JR.theme);
    ...
    JR.styleEditor = editor;

    JR.datGUI = new dat.GUI();

    const themeList = JR.datGUI.add(
      editor,
      "theme",
      Object.keys(JR.stylesheetObj),
    );
    editor.theme = JR.theme;

    prevStyles.originalThemeCSS = JR.stylesheetObj[JR.theme];
    themeList.onChange((value) => {
      persistCurrentTheme(value);
